...
const AnimatedPatrollerMap = ({ navigation, route }) => {
  ...
  const markerPosition = new Animated.ValueXY();
  ...
  const renderRoute = async () => {
    try {
      ...
      if (data.status === "OK") {
        ...
        animateMarkerMovement(routeCoordinates);
      } else {
  ...
  const animateMarkerMovement = (coordinates) => {
    Animated.timing(markerPosition, {
      toValue: { x: coordinates[0].longitude, y: coordinates[0].latitude },
    ...
    >
      {patrollerLocation && (
        <>
          ...
          <Marker.Animated coordinate={markerPosition} title="Patroller" />
...
export default AnimatedPatrollerMap;
